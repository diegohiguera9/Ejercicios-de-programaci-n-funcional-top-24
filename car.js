...
// 2.1 Create a function ferraris that returns all the cars of the Ferrari brand

function ferraris (cars){    
    return cars.filter((elemento)=>{
        if (elemento.brand === 'Ferrari'){return true};    
    });
};

let ferrari = ferraris(cars);
console.log(ferrari);

// 2.2 Create a function nineties that returns all the cars with models between 1980 and 1990.

function nineties(cars){
    return cars.filter((elemento)=>{
        if (elemento.model>=1980 && elemento.model<=1990){return true};
    });
};

let ninety = nineties(cars);
console.log(ninety)

// 2.3 Create a function list that returns an array of strings with the same number of cars. 
...
function list(cars){
    return cars.map((elemento)=>{
        return `This ${elemento.brand} from ${elemento.model} costs ${elemento.price}`
    });
};
...
// 2.4 Create a function bmwSum that returns the sum of all prices of the BMW cars

function bmwSum(cars){
    let bmwPrice = [];
    cars.forEach((elemento)=>{
        if(elemento.brand === 'BMW'){bmwPrice.push(elemento.price)};
    });

    return bmwPrice.reduce((acum,curr)=>
        acum + curr, 0
    );
};
